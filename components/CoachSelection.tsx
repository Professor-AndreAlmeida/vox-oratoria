...
export const NewSessionSetup: React.FC<NewSessionSetupProps> = ({ onStartPractice, onBack }) => {
    ...
    const handleStart = () => {
        onStartPractice(topic || 'Sessão Livre', script, analysisMode);
    };

    // FIX: Add a loading state while user is being fetched.
    if (!user) {
                 ...
                 <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                        {analysisModes.map(mode => (
                            ...
                            >
                                {/* FIX: Cast icon to a type that explicitly includes className to resolve TypeScript error. */}
